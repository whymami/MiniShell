...
void set_hrd_cmd(t_minishell *mini)
{
    ...
    cmd = ft_strtrim(cmd, " ");
    if (!mini->hdc_cmd)
        mini->hdc_cmd = dlist_new(cmd);
    else
        dlist_add_back(&mini->hdc_cmd, dlist_new(cmd));
    free(cmd);
}
...
int heredoc(t_minishell *mini)
{
    ...
    if (!delimiter)
        return (ft_printf("%ssyntax error near unexpected token `newline'\n", ERR_TITLE), 1); // komut yoksa yani " << eof" gibi bir durumda executor komut yok hatası bastırmamalı.
    while (1)
    {
        line = readline("> ");
        if (!line)
