...
/*   Updated: 2024/07/27 19:21:51 by muguveli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	rdirect_out(char *file, int append)
{
	...
	if (append)
	{
		fd = open(file, O_CREAT | O_WRONLY | O_APPEND, 0644);
			// if'e süslü parantez koymamın sebebi warning vermesi ft_rdirectte silmeyin
	}
	else
	{
...
static int	ft_rdirect(t_minishell *mini, char **args)
{
	...
	while (args[j])
	{
		if ((ft_strcmp(args[j], ">") == 0 && args[j + 1]) || (ft_strcmp(args[j],
					">>") == 0 && args[j + 1]))
		{
			file = args[j + 1];
